...
@section('admin-content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <form method="POST" action="{{ route('admin.terrains.store') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="number" id="prix_heure" name="prix_heure" value="{{ old('prix_heure') }}"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 @error('description') border-red-500 @enderror"
                              placeholder="Décrivez les caractéristiques du terrain...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
